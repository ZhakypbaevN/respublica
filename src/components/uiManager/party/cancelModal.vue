...
const postCencelExitParty = ({ motive }: { motive: string }) => {
  loading.value = true;
  const url = `https://api.respublica.codetau.com/api/v1/admin/parties/memberships/resignations/${props.id}/reject`

  const formData = new FormData();
  
  formData.append("reject_data", JSON.stringify({reason_for_reject: motive}));
